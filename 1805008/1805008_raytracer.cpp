...
RayTracer::RayTracer(std::ifstream &f){
    ...
    board = CheckerBoard::parseCheckerBoard(f);
...
void RayTracer::generatePointBuffer(const CameraConfig &cConfig){
    ...
    std::cout<< "Point Buffer generated\n";
...
void RayTracer::generateImage(const CameraConfig &cConfig){
    ...
    for(int i = 0; i < config.w; ++i){
        ...
        double ratio = (c / tot) * 10;
        int index = ratio;
        if(index > 0 && index < 10){
            if(!printed[index]){
                std::cout << index * 10 << "% done\n";
                printed[index] = true;
            }
        }
