...
public:
    ...
    void clipAndScanBuffer(vector<Matrix> &trMs){
        ...
        for(auto tr : trs){
            ...
            int bottomIndex = floor(abs(mnY - 1) / dy);
            ...
            for(int i = topIndex; i < bottomIndex; ++i){
                ...
                left = max(left, -1.);
                right = min(right, 1.);
                int leftIndex = floor((left + 1.) / dx);
                int rightIndex = floor((right + 1.) / dx);
        ...
        for(int i = 0; i < h; ++i){
            for(int j = 0; j < w; ++j){
                if(buffer[i][j] == 1.)
                    ...
                    zbuffer << fixed << setprecision(6)<< buffer[i][j] << "   " ;
            }
            zbuffer << "\n";
        }

        image.save_image(outPath + "/out.bmp");
    }
...
int main(int argc, char** argv){
    // string dir = "./TestCases__/" + tc;
    string dir = ".";
    // if(argc > 1){
    //     dir = argv[1];
    // }
    // Solver solve(dir, "./output/" + tc);
    Solver solve(dir, ".");
